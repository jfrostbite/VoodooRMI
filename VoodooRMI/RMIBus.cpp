...
#define super IOService
  ...
  updateConfiguration(
      OSDynamicCast(OSDictionary, getProperty("Configuration")));
  return true;
}
...
bool RMIBus::start(IOService *provider) {
  ...
  if (!transport->open(this, 0,
                       OSMemberFunctionCast(RMIAttentionAction, this,
                                            &RMIBus::handleHostNotify))) {
...
err:
  ...
  while (RMIFunction *func =
             OSDynamicCast(RMIFunction, iter->getNextObject())) {
    func->stop(this);
    func->detach(this);
  }
...
void RMIBus::notify(UInt32 type, void *argument) {
  if (type == kHandleRMIClickpadSet || type == kHandleRMITrackpoint) {
...
IOReturn RMIBus::setProperties(OSObject *properties) {
  commandGate->runAction(OSMemberFunctionCast(IOCommandGate::Action, this,
                                              &RMIBus::updateConfiguration),
                         OSDynamicCast(OSDictionary, properties));
  publishVoodooInputProperties();
  return kIOReturnSuccess;
}
...
void RMIBus::updateConfiguration(OSDictionary *dictionary) {
  ...
  update |= Configuration::loadUInt32Configuration(
      dictionary, "TrackpointMultiplier", &conf.trackpointMult);
  update |= Configuration::loadUInt32Configuration(
      dictionary, "TrackpointScrollMultiplierX", &conf.trackpointScrollXMult);
  update |= Configuration::loadUInt32Configuration(
      dictionary, "TrackpointScrollMultiplierY", &conf.trackpointScrollYMult);
  update |= Configuration::loadUInt32Configuration(
      dictionary, "TrackpointDeadzone", &conf.trackpointDeadzone);
  update |= Configuration::loadUInt64Configuration(
      dictionary, "DisableWhileTypingTimeout", &conf.disableWhileTypingTimeout);
  update |= Configuration::loadUInt64Configuration(
      dictionary, "DisableWhileTrackpointTimeout",
      &conf.disableWhileTrackpointTimeout);
  update |= Configuration::loadUInt32Configuration(
      dictionary, "ForceTouchMinPressure", &conf.forceTouchMinPressure);
  update |= Configuration::loadUInt32Configuration(
      dictionary, "ForceTouchType",
      reinterpret_cast<UInt32 *>(&conf.forceTouchType));
  update |= Configuration::loadUInt32Configuration(
      dictionary, "MinYDiffThumbDetection", &conf.minYDiffGesture);
  update |= Configuration::loadUInt8Configuration(
      dictionary, "PalmRejectionMaxObjWidth", &conf.palmRejectionMaxObjWidth);
  update |= Configuration::loadUInt8Configuration(
      dictionary, "PalmRejectionMaxObjHeight", &conf.palmRejectionMaxObjHeight);
  update |= Configuration::loadUInt8Configuration(
      dictionary, "PalmRejectionWidth", &conf.palmRejectionWidth);
  update |= Configuration::loadUInt8Configuration(
      dictionary, "PalmRejectionHeight", &conf.palmRejectionHeight);
  update |= Configuration::loadUInt8Configuration(
      dictionary, "PalmRejectionTrackpointHeight",
      &conf.palmRejectionHeightTrackpoint);
  update |= Configuration::loadUInt32Configuration(
      dictionary, "FingerTrackingMaxDistance", &conf.fingerTrackingMaxDistance);
...
void RMIBus::getGPIOData(OSDictionary *dict) {
  ...
  Configuration::loadBoolConfiguration(dict, "TrackstickButtons",
                                       &gpio.trackpointButtons);
...
void RMIBus::publishVoodooInputProperties() {
  ...
  if (trackpointFunction != nullptr) {
    ...
    setPropertyNumber(trackpoint, VOODOO_TRACKPOINT_DEADZONE,
                      conf.trackpointDeadzone, 32);
    setPropertyNumber(trackpoint, VOODOO_TRACKPOINT_BTN_CNT, 3, 32);
    setPropertyNumber(trackpoint, VOODOO_TRACKPOINT_MOUSE_MULT_X,
                      conf.trackpointMult, 32);
    setPropertyNumber(trackpoint, VOODOO_TRACKPOINT_MOUSE_MULT_Y,
                      conf.trackpointMult, 32);
    setPropertyNumber(trackpoint, VOODOO_TRACKPOINT_MOUSE_DIV_X, DEFAULT_MULT,
                      32);
    setPropertyNumber(trackpoint, VOODOO_TRACKPOINT_MOUSE_DIV_Y, DEFAULT_MULT,
                      32);
    setPropertyNumber(trackpoint, VOODOO_TRACKPOINT_SCROLL_MULT_X,
                      conf.trackpointScrollXMult, 32);
    setPropertyNumber(trackpoint, VOODOO_TRACKPOINT_SCROLL_MULT_Y,
                      conf.trackpointScrollYMult, 32);
    setPropertyNumber(trackpoint, VOODOO_TRACKPOINT_SCROLL_DIV_X, DEFAULT_MULT,
                      32);
    setPropertyNumber(trackpoint, VOODOO_TRACKPOINT_SCROLL_DIV_Y, DEFAULT_MULT,
                      32);
  ...
  messageClient(kIOMessageVoodooTrackpointUpdatePropertiesNotification,
                voodooInputInstance);
}
